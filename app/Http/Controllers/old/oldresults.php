...
class ResultController extends Controller
{
    ...
    public function list(){
        ...
        $results = DB::select("select 
                                ...
                                from (
                                    select home_team as team_name, home_logo_url as logo_url, score_home as goalsfor, score_away as goalsagainst from results where score_home <> 0 OR score_away <> 0
                                union all
                                    select away_team, away_logo_url, score_away as goalsagainst, score_home as goalsfor from results where score_home <> 0 OR score_away <> 0
                                ) a
                                group by team_name
        ...
        $results2 = DB::select("select 
                                ...
                                0 score 
                            from (
                                select home_team as team_name, home_logo_url as logo_url, score_home as goalsfor, score_away as goalsagainst from results where score_home = 0 AND score_away = 0
                            union all
                                select away_team, away_logo_url, score_away as goalsagainst, score_home as goalsfor from results where score_home = 0 AND score_away = 0
                            ) a
                            group by team_name 
